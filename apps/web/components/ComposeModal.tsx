    ...
    <div className="compose-page">
      <form className="compose-form" onSubmit={submit}>
        ...
        <div className="compose-fields">
          ...
          <div className="compose-line settings-line"><span className="sr-only">Options</span><div><label>Delay between 2 emails<input type="number" min={DEFAULT_MIN_DELAY_MS} value={delay} onChange={event => setDelay(Number(event.target.value))} /></label><label>Hourly Limit<input type="number" min="1" value={limit} onChange={event => setLimit(Number(event.target.value))} /></label><span className="recipient-count">{recipients.length} recipients</span></div></div>
